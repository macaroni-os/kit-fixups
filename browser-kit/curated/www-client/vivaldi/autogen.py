...
async def generate(hub, **pkginfo):
	...
	artifacts = [
		...
		hub.pkgtools.ebuild.Artifact(hub, url=url.replace("amd64", "arm64"), final_name=f"vivaldi-{version}-arm64.deb"),
		hub.pkgtools.ebuild.Artifact(hub, url=url.replace("amd64", "armhf"), final_name=f"vivaldi-{version}-armhf.deb"),
		hub.pkgtools.ebuild.Artifact(hub, url=url.replace("amd64", "i386"), final_name=f"vivaldi-{version}-i386.deb")
	]
