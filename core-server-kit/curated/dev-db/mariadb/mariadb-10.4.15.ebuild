...
inherit eutils systemd flag-o-matic prefix toolchain-funcs \
        java-pkg-opt-2 user cmake cmake-utils multilib-build
...
"
DEPEND="virtual/yacc
    static? ( sys-libs/ncurses[static-libs] ) || ( >=sys-devel/gcc-3.4.6 >=sys-devel/gcc-apple-4.0 )    
    	server? (
...
pkg_config() {
	...
	if [[ -n "${MYSQL_ROOT_PASSWORD}" ]] ; then
		...
		local sql="ALTER USER 'root'@'localhost' IDENTIFIED BY '${MYSQL_ROOT_PASSWORD}';FLUSH PRIVILEGES;"
		"${EROOT}/usr/bin/mysql" \
			"--socket=${socket}" \
			-hlocalhost \
			-e "${sql}"
