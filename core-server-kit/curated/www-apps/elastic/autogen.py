...
async def generate(hub, **pkginfo):
	...
	packages = {
		...
		"app-misc/elasticsearch": "elasticsearch",
	}

	for package in packages:
		github_repo = packages[package]
		...
		for major in [latest.major, latest.major - 1, latest.major - 2]:
			...
			pkginfo["cat"], pkginfo["name"] = package.split("/")
			name = pkginfo["name"].replace("-bin","")

			# find available architecture tarballs
			download_page = f"{download_url}/{name}-{version.public.replace('.', '-')}"
			...
			#if "search" in pkginfo["name"]:
			#	pkginfo["template"] = pkginfo["name"] + template + ".tmpl"
