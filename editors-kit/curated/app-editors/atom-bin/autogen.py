...
async def generate(hub, **pkginfo):
	...
	json_dict = await hub.pkgtools.fetch.get_page(
		"https://api.github.com/repos/atom/atom/releases", is_json=True, refresh_interval=timedelta(days=5)
	)
	...
	if r:
		dl_assets = list(
			filter(
				lambda x: x["browser_download_url"].endswith("-amd64.deb"),
				r["assets"],
			)
		)
		if len(dl_assets):
	...
	if release is None or dl_asset is None:
		raise hub.pkgtools.ebuild.BreezyError("Can't find a suitable release of Atom.")
