...
async def generate(hub, **pkginfo):
    id_key = "abcde"
    pkginfo["version"] = "1.0"
    my_archive = hub.Archive.find(f"dynamic-archive-1.0-{id_key}.tar.xz", key={"id_key": id_key})
    if not my_archive:
        ...
        with open(os.path.join(my_archive.top_path, "README"), "w") as myf:
            myf.write("HELLO")
        my_archive.store(key={"id_key" : id_key})
    my_key_archive = my_archive

    my_archive = hub.Archive.find("dynamic-archive-1.0.tar.xz")
    if not my_archive:
