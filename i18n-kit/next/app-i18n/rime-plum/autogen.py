...
async def process_pkg(pkg, hub, **pkginfo):
    commits = await query_github_api(hub, github_user, f"{github_repo}-{pkg}", "commits?sha=master")
    valid_commits = (
        commit for commit in commits if await is_valid(hub, github_user, f"{github_repo}-{pkg}", commit)
    )
...
async def generate(hub, **pkginfo):
    commits = await query_github_api(hub, github_user, "plum", "commits?sha=master")

    valid_commits = (
        commit for commit in commits if await is_valid(hub, github_user, "plum", commit)
    )
