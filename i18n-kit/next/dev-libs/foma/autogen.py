...
async def query_github_api(hub, user, repo, query):
...
async def is_valid(hub, user, repo, commit):
    commit_sha = commit["sha"]
    commit_status = await query_github_api(hub, user, repo, f"commits/{commit_sha}/status")

    return "failure" not in commit_status["state"]


async def get_latest_commit(hub, github_user, github_repo, query):
    commits = await query_github_api(hub, github_user, github_repo, query)

    valid_commits = (
        commit for commit in commits if await is_valid(hub, github_user, github_repo, commit)
    )
...
async def generate(hub, **pkginfo):
    ...
    target_commit = await get_latest_commit(hub, github_user, github_repo, "commits?path=foma/CHANGELOG&sha=master")
    ...
    if date == "20210601":
        target_commit = await get_latest_commit(hub, github_user, github_repo, "commits?sha=master")
