...
UPSTREAM_BOOTSTRAP_VERSION="$(ver_cut 1).$(ver_cut 2).1"
...
# https://bugs.funtoo.org/browse/FL-10975
# As of Rust 1.65.0, upstream's bundled LLVM has been upgraded to LLVM 15
# Disabling system-llvm USE flag until in-tree LLVM is upgraded to 15 or beyond
# Warning: if you use system-llvm and link rustc against sys-devel/llvm-13.0.0-r1
# you will experience Rust Crate compilation errors and break rustc
IUSE="clippy cpu_flags_x86_sse2 debug dist doc miri nightly parallel-compiler profiler rustfmt rust-src bash-completion +system-bootstrap upstream-bootstrap -system-llvm test wasm ${ALL_LLVM_TARGETS[*]}"
...
src_configure() {
	...
	cat <<- _EOF_ > "${S}"/config.toml
		...
		# https://github.com/rust-lang/rust/pull/101403/files
		lto = "thin"
		[dist]
		src-tarball = false
