...
async def get_translations(hub, user, repo, version):
    translations_dict = await hub.pkgtools.fetch.get_page(
        f"https://api.github.com/repos/{user}/{repo}/contents/data/translations?ref={version}",
        is_json=True)
    translations_format = "Internationalization_(.*).ts"
    translations_list = [re.search(translations_format, file_entry["name"]).group(
        1) for file_entry in translations_dict]
