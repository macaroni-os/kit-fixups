...
async def generate(hub, **pkginfo):
	...
	if 'version' not in pkginfo or 'latest' in pkginfo['version']:
		...
		files = pkginfo['name']
		files += '|'.join(pkginfo["dir"]["files"]) if 'files' in pkginfo else ''
		releases = re.findall(
			f'(?<=href=")({files})-\d+(?:\.\d+)+(?=\.tar\.gz"|\.tar\.bz2"|\.tar\.xz|\.zip")',
			release_data
			)
...
# look for all files in the same dir
	artifacts = dict()
	if 'files' in pkginfo['dir']:
		artifacts = {
			f : hub.pkgtools.ebuild.Artifact(
				url = f"{pkginfo['dir']['url']}{f}-{pkginfo['version']}"
					+ f".{pkginfo['dir']['format']}"
			) for f in pkginfo['dir']['files']
		}
	else:
		artifacts = {
			pkginfo['name'] : hub.pkgtools.ebuild.Artifact(
				url = f"{pkginfo['dir']['url']}{pkginfo['name']}-{pkginfo['version']}"
					+ f".{pkginfo['dir']['format']}"
			)
		}
