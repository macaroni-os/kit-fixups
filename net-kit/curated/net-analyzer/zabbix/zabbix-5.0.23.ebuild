...
PATCHES=(
	"${FILESDIR}/5.0/patches/${PN}-4.0.18-modulepathfix.patch"
	"${FILESDIR}/5.0/patches/${PN}-5.0.22-system.sw.packages.patch"
...
src_install() {
	...
	if use server; then
		...
		newinitd "${FILESDIR}"/5.0/zabbix-server.init zabbix-server
		...
		systemd_dounit "${FILESDIR}"/5.0/zabbix-server.service
		systemd_newtmpfilesd "${FILESDIR}"/5.0/zabbix-server.tmpfiles zabbix-server.conf
	fi
	...
	if use proxy; then
		...
		newinitd "${FILESDIR}"/5.0/zabbix-proxy.init zabbix-proxy
		...
		systemd_dounit "${FILESDIR}"/5.0/zabbix-proxy.service
		systemd_newtmpfilesd "${FILESDIR}"/5.0/zabbix-proxy.tmpfiles zabbix-proxy.conf
	fi
	...
	if use agent; then
		...
		newinitd "${FILESDIR}"/5.0/zabbix-agentd.init zabbix-agentd
		...
		systemd_newtmpfilesd "${FILESDIR}"/5.0/zabbix-agentd.tmpfiles zabbix-agentd.conf
	fi
	...
	if use java; then
		...
		newinitd "${FILESDIR}"/5.0/zabbix-jmx-proxy.init zabbix-jmx-proxy
		newconfd "${FILESDIR}"/5.0/zabbix-jmx-proxy.conf zabbix-jmx-proxy
	fi
}
