...
LANG_SUPPORT_EXTENDED=( go lua php pypy python python_asyncio python_gevent python_greenlet ruby )
...
REQUIRED_USE="|| ( ${LANG_SUPPORT_SIMPLE[@]} ${LANG_SUPPORT_EXTENDED[@]} )
	...
	python_asyncio? ( python_greenlet || ( python_targets_python3_4 python_targets_python3_5 python_targets_python3_6 ) python_gevent )
...
CDEPEND="sys-libs/zlib
	...
	python_greenlet? ( dev-python/greenlet[${PYTHON_USEDEP}] )
...
src_install() {
	...
	newinitd "${FILESDIR}"/uwsgi.initd-r8 uwsgi
	newconfd "${FILESDIR}"/uwsgi.confd-r5 uwsgi
	keepdir /etc/"${PN}".d
...
pkg_postinst() {
	...
	python_pkg_postinst() {
		...
		if use python_asyncio ; then
			elog "  '--plugins ${EPYV},asyncio${PYV},greenlet${PYV}' for asyncio support in ${EPYTHON}"
		fi
