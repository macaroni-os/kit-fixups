...
PATCHES=( \
		...
		"${FILESDIR}"/lic.patch \
		"${FILESDIR}"/gle-4.3.3-missing-dl-lib.patch \
		"${FILESDIR}"/gle-4.3.3-qgle-shell-script.patch \
	)
...
src_install() {
	...
	newbin gui/qgle qgle.bin
	insinto /usr/share/${PN}
	doins gle/glerc
	doins -r gle/font
	popd > /dev/null
	newbin "${FILESDIR}"/gle.sh gle
	newbin gui/start-qgle.sh qgle
