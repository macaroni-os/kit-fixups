...
def extract_urls(tag, pattern):
    return list(filter(None, [pattern.match(t.get("href"))
...
def find_translations(hub, name, language_list, url_pattern):
    ...
    if not translation_matches:
        raise hub.pkgtools.ebuild.BreezyError(
            f"Can't find translations for {name}!")
    ...
    translation_artifacts = [hub.pkgtools.ebuild.Artifact(
        url=url) for url in translation_urls]
...
async def generate(hub, **pkginfo):
    ...
    download_page_data = await hub.pkgtools.fetch.get_page(
    ...
    url_pattern = re.compile(
        r"(https:\/\/dl\.xpdfreader\.com\/xpdf-(.*)\.tar\.gz)")
